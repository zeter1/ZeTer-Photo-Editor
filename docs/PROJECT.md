...
| Text add/edit transaction и live preview | `src/ui/text-edit-controller.js`; shared typography/font UI policy — `src/ui/text-settings-controller.js`; persisted text schema/preview projection — `src/core/state.js`; rendering/font loading — `src/core/render.js` | `tests/text-edit-controller.test.mjs`, `tests/text-settings-controller.test.mjs`, `tests/text-font.test.mjs`, `tests/render-pipeline.test.mjs`, browser smoke |
...
| Layers panel/tree: recursive rows, ARIA/focus, thumbnails/mask hints, panel DnD/root drop | `src/ui/layers-panel-controller.js`; mutations/schema — `src/core/state.js`; feature menus/history — runtime ports in `src/main.js` | `tests/layers-panel-controller.test.mjs`, `tests/layer-groups.test.mjs`, architecture test, browser smoke |
| Saved Paths: selection, CRUD, panel/context menu, apply as vector mask | `src/ui/paths-controller.js`; Pen direct-edit geometry stays in `src/main.js` | `tests/paths-controller.test.mjs`, `tests/paths-panel.test.mjs`, `tests/vector-masks.test.mjs`, workspace-session tests |
| Документы, слои, groups, persisted Smart Object schema | `src/core/state.js` | core/layer/smart-object tests |
| Smart Object content lifecycle: convert/open/save/link/unlink, shared-source propagation, content-tab guards | `src/document/smart-object-controller.js`; Photoshop resource owner подключён narrow ports | `tests/smart-object-controller.test.mjs`, `tests/linked-smart-objects.test.mjs` |
| Photoshop Smart Object embedded resource rewrite: PNG/PSD/PSB payload, `liFD` prepare/publish, baseline fingerprints | `src/document/psd-smart-object-resource.js`; export preparation: `src/document/psd-export-controller.js`; binary codec: `src/formats/psd.js` | `tests/psd-smart-object-resource.test.mjs`, `tests/psd-smart-object-roundtrip.test.mjs`, `tests/psd-export-integration.test.mjs` |
...
| Selection → raster layer masks / Select & Mask refine + preview | `src/selection/mask-controller.js`; refine/compositor math stays in `src/core/pixels.js`; Smart Filter mutation stays in `src/ui/smart-filter-controller.js` | `tests/selection-mask-controller.test.mjs`, `tests/selection-refine.test.mjs`, `tests/smart-filter-controller.test.mjs` |
| Selection → Vector Mask create/boolean/edit-entry/toggle/invert/remove | `src/selection/vector-mask-controller.js`; Pen direct-edit geometry/state stays in `src/main.js`; Saved Paths in `src/ui/paths-controller.js`; PSD bridge/codec remain separate | `tests/selection-vector-mask-controller.test.mjs`, `tests/vector-masks.test.mjs`, paths/PSD regressions |
...
- `src/ui/modal-controller.js` — generic modal/dialog shell: form fields, numeric normalization, focus restore, backdrop/Escape close, draggable text-modal lifecycle, info dialogs и recovery project-manager dialog; feature-specific state не хранит и получает только узкие callbacks.
- `src/ui/text-edit-controller.js` — Text add/edit transaction owner: exact target guards, transient draft, latest-wins async preview и preview-canvas lifecycle.
- `src/ui/text-settings-controller.js` — shared Text typography/font UI-policy owner: option sets, local-font discovery/private registry, custom-font cache/validation, modal fields и form normalization. Properties использует его через narrow methods, но DOM/history orchestration остаётся в `src/main.js`.
...
- `src/ui/layers-panel-controller.js` — owner DOM/presentation lifecycle панели Layers: recursive tree ordering, selected/effective visibility-lock presentation, thumbnails/mask hints, row-local keyboard focus и layer/group DnD including root drop. Он хранит drag identity локально и отбрасывает callbacks от уже неактивного document; schema/mutation primitives остаются в `src/core/state.js`, history + feature-heavy context-menu commands — runtime ports.
- `src/ui/paths-controller.js` — Saved Paths orchestration owner: selected path index, Photoshop-compatible resource allocation/limit, CRUD, accessible panel/context-menu wiring и apply-as-vector-mask через grouped injected ports; не владеет Pen geometry или PSD binary codec.
- `src/ui/color-management-controller.js` — document CMYK/ICC orchestration owner: preview/edit transform caches, policy/profile transactions, async preview rebuild + stale-document rollback и properties-panel bindings. ICC parsing/transform math остаётся в `src/core/color-management.js`; PSD/PSB byte semantics — в `src/formats/psd.js`.
...
- `src/workspace/recovery-controller.js` — application-level recovery owner: per-window key ownership, debounce/write serialization, dirty-session snapshots, startup saved-project listing/restore/delete policy, explicit cross-window delete confirmation and preservation of unreadable sibling snapshots. It receives storage/project/session/UI ports explicitly; IndexedDB records stay in `src/core/recovery.js`.
...
- `src/selection/mask-controller.js` — владелец selection → raster-mask / Select & Mask orchestration: mask rasterization for layer/document coordinates, bounded refine preparation, non-destructive modal preview, add/remove/final Apply transactions and exact document + selected-layer + lock revalidation after awaits. `src/core/pixels.js` owns refine math; Smart Filter reuses only `selectionMaskDataUrl()` and keeps its own mutation/publication owner.
- `src/selection/vector-mask-controller.js` — владелец selection → Vector Mask command/policy seam: exact rect/ellipse/path geometry projection, layer-local localization, boolean subpath publication, 128-path guard and mask lifecycle commands. Он вызывает Pen edit только через narrow runtime port; `vectorMaskEditLayerId`, Saved Path edit state, hit/drag geometry and PSD import/export bridges остаются вне controller.
- `src/document/smart-object-controller.js` — Smart Object content/session lifecycle owner: convert/open/save/link/unlink, shared-source propagation, nesting/source-bounds rules и async content-tab stale guards. Browser preview rendering, session/runtime publication и Photoshop embedded-resource rewrite приходят через узкие ports; PSD/PSB bytes остаются вне generic controller.
...
- `src/document/psd-import-controller.js` — канонический owner PSD/PSB import transaction: file guard, decode orchestration, decoded layer/group/path mapping, high-depth/CMYK preservation, ICC-preview policy, stale-document guard и публикация через runtime port. Binary parser остаётся в `src/formats/psd.js`.
- `src/document/psd-import-semantics.js` — Photoshop-specific import semantics для Text/Shape/Adjustment/Smart Object и editable embedded assets. Stable core/domain contracts импортируются напрямую; browser decode/preview и shared vector-mask/opaque-resource bridges приходят явными ports. Session/history/publish и binary writer здесь не живут.
- `src/document/psd-native-metadata-plans.js` — канонический owner Photoshop-native export eligibility/rewrite planning для Text/TySh, solid Shape, Adjustment и Smart Object metadata; вызывает только публичные rewrite primitives формата и не пишет PSD/PSB layout.
- `src/document/psd-export-controller.js` — канонический owner document→PSD/PSB writer preparation: bounds, group lineage, native 16/32-bit/CMYK eligibility, raster/native payloads, merged composite and export warnings. Metadata plans импортируются напрямую из соседнего document owner; browser rendering и generic vector-mask bridge остаются узкими ports.
- `src/formats/psd.js` — канонический PSD/PSB binary codec и Photoshop metadata parse/write/rewrite-primitive boundary.
