...
- PSD/PSB Group Export Stage 8b: writer принимает flat `groups` metadata и разворачивает каждый contiguous group run в hidden bounding divider + folder `lsct` record. Folder visibility/collapsed-state пишутся отдельно от child layer flags, поэтому скрытие группы не уничтожает индивидуальную видимость детей. Несмежная группа считается нарушением структурного invariant и отклоняется `PSD_EXPORT_GROUP_SPLIT`; nested group export ждёт расширения ZPE schema.
