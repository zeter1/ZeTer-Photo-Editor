...
| PSD/PSB import/export | import transaction/mapping: `src/document/psd-import-controller.js`; export preparation: `src/document/psd-export-controller.js`; binary codec: `src/formats/psd.js` | `tests/psd-import-controller.test.mjs`, `tests/psd-export-controller.test.mjs`, `tests/psd-*.test.mjs` |
...
- `src/document/psd-export-controller.js` — канонический owner document→PSD/PSB writer preparation: bounds, group lineage, native 16/32-bit/CMYK eligibility, raster/native payloads, merged composite and export warnings. Получает Photoshop metadata-plans и rendering через явные ports; binary bytes не пишет.
- `src/formats/psd.js` — канонический PSD/PSB binary codec и Photoshop metadata parse/write/rewrite boundary.
