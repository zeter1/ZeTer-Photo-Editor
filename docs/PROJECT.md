...
| PSD/PSB import/export | export preparation: `src/document/psd-export-controller.js`; binary codec: `src/formats/psd.js`; import mapping/wiring: bounded PSD section in `src/main.js` | `tests/psd-export-controller.test.mjs`, `tests/psd-*.test.mjs` |
...
- `src/formats/psd.js` — канонический PSD/PSB binary codec и Photoshop metadata parse/write/rewrite boundary. Import mapping пока остаётся bounded runtime seam в `src/main.js`.
