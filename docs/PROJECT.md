...
`npm run check` пересобирает bundle, проверяет его синтаксис и запускает все Node regression-тесты. Для разработки через HTTP существует `npm run serve`; пользовательский запуск на Windows выполняется через `start.bat` или `index.html`.
...
- Состояние проекта хранится в `.zpe`; аварийное восстановление использует IndexedDB: каждое окно хранит отдельный снимок всех своих несохранённых вкладок, а при запуске доступны все копии, включая старый одиночный формат. «Позже» оставляет найденные копии без удаления. Скачивание `.zpe` через браузер не подтверждает, что файл появился на диске; до проверки файла вкладка остаётся помеченной несохранённой.
...
- Node-тесты не доказывают внешний вид, реальные pointer-события, Clipboard API или поведение конкретного браузера при `file://`; эти границы требуют browser smoke.
