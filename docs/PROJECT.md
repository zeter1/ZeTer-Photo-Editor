...
- PSD Blob Stage 6c: UI использует `encodePsdBlob()` и строит Blob прямо из writer chunks; `encodePsd()` остаётся byte-array compatibility API. Это убирает обязательную финальную contiguous-копию в browser download path, но настоящий `WritableStream`/PSB >2 GB streaming ещё не заявляется.
