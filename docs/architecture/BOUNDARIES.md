...
- Text Properties may consume settings-controller methods, but its markup/history orchestration stays in `src/main.js`. Any awaited custom-font publication must revalidate the originating document, exact selected layer identity and lock state before mutation/commit.
...
- Raster-mask schema stays in `src/core/state.js`; refine/edge/compositor math stays in `src/core/pixels.js`; Smart Filter mutation stays in `src/ui/smart-filter-controller.js`; Vector Mask/Pen semantics remain separate.
