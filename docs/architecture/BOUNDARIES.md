...
- Vector Mask Pen edit identity, Saved Path edit identity, anchor/handle hit-drag mechanics and overlay routing remain in `src/main.js`; Saved Paths ownership remains in `src/ui/paths-controller.js`; PSD/PSB vector-mask import/export conversion stays outside the selection controller. Entering Vector Mask edit must clear a competing Saved Path edit target before publishing the Vector Mask target.
