...
- **PSD/PSB Group Import Stage 8a**: `lsct` open/closed-folder и bounding-divider records восстанавливаются в группы ZPE; сохраняются принадлежность слоёв, видимость и collapsed-state. Так как текущая модель групп ZPE плоская, вложенные Photoshop-группы импортируются как отдельные группы с полным путём (`Parent / Child`) в имени;
- **PSD/PSB Group Export Stage 8b**: плоские группы ZPE экспортируются как настоящие Photoshop `lsct` folder/bounding records с Unicode-именем, visibility и open/closed state; индивидуальная видимость дочерних слоёв не теряется при скрытой группе. Несмежные члены одной группы отклоняются отдельной диагностикой вместо захвата посторонних слоёв;
