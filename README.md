...
Самый простой вариант: дважды кликните `start.bat` или сразу `index.html`. Версия 1.39.0 специально собрана так, чтобы работать через обычный `file://` запуск — **Python, Node.js и локальный HTTP-сервер для использования редактора больше не нужны**.
...
PSD Import Stage 3 / Export Stage 4 и PSB Stage 7a дают layered round-trip, Stage 12e–12g сохраняют native RGB 16/32-bit typed channels и compatible high-depth composite, Stage 13a–13e закрывают native CMYK/color-management и real-world ICC corpus, Stage 14a–14c закрывают основную embedded Smart Object цепочку, Stage 15a–15b дают editable Photoshop text, а Stage 15c–15d дают editable solid Shape Layer с native path и descriptor rewrite для fill/stroke/width. Gradient/Pattern Fill metadata уже разбираются и удерживаются bounded, но ещё не имеют собственного editable renderer/writer; multi-subpath boolean geometry и scale/rotation semantics тоже пока fallback. Photoshop Adjustment Layer mapping, полная parity isolated groups/vector-mask composites и tiled/streaming pipeline ещё впереди.

Следующий imaging-этап: **Stage 16a — Photoshop-native Adjustment Layer Mapping**: начать с хорошо определённых adjustment records (Brightness/Contrast, Exposure, Hue/Saturation, Levels/Curves), импортировать их в ZPE adjustment layers с bounded native metadata и safe pass-through/writeback, а неподдержанные варианты оставлять на raster/composite fallback. После этого — isolated group/vector-mask parity и tiled/worker/GPU pipeline.