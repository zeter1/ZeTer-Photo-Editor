...
PSD Import Stage 3 / Export Stage 4 и PSB Stage 7a дают layered round-trip, Stage 12e–12g сохраняют native RGB 16/32-bit typed channels и compatible high-depth composite, Stage 13a–13e закрывают native CMYK/color-management и real-world ICC corpus, Stage 14a–14c закрывают основную embedded Smart Object цепочку, а Stage 15a уже импортирует horizontal Photoshop `TySh` как editable text layer и сохраняет его native descriptor при неизменной typography. Для текста пока не переписываются `EngineData` run/style/paragraph structures после content/style edit; vertical text тоже остаётся raster fallback. Shape/Vector Fill, Adjustment Layer mapping, полная parity isolated groups/vector-mask composites и tiled/streaming pipeline ещё впереди.

Следующий imaging-этап: **Stage 15b — Photoshop EngineData Typography & Editable Text Writeback**: разобрать `EngineData` font/style/paragraph runs, применить first-run/default typography к ZPE text layer и безопасно переписывать `Txt ` + `/Editor /Text` + совместимые run lengths при редактировании текста. Затем — Shape/Vector Fill и Adjustment Layer mapping.