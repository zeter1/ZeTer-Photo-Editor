...
Самый простой вариант: дважды кликните `start.bat` или сразу `index.html`. Версия 1.24.0 специально собрана так, чтобы работать через обычный `file://` запуск — **Python, Node.js и локальный HTTP-сервер для использования редактора больше не нужны**.
...
Следующие крупные направления развития: **Stage 12d native high-depth destructive editing foundation** (pixel operations над `Uint16Array`/`Float32Array` без раннего перехода в Canvas8), затем PSD/PSB 16/32-bit writer/export, CMYK/ICC color-management pipeline, PSD/PSB mapping для embedded smart objects/text/vector/adjustment layers, точная Photoshop parity для сложных group/adjustment interactions, внешние file-linked Smart Objects и Photoshop-native Smart Filter round-trip, clipping-path designation и richer Photoshop vector-origination metadata, tiled/streaming storage и GPU/worker-ускорение для очень больших документов.