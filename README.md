...
- **Перо (`P`)**, Stage 2a: клик создаёт угловой узел, click+drag — гладкий Bézier-узел с симметричными ручками, `Alt+drag` — угловой узел с независимой исходящей ручкой; cubic-контуры сохраняются в `.zpe`, а старые прямолинейные pathPoints остаются совместимыми; **Магнитное лассо (`A`)** прослеживает границу промежуточными точками между кликами, а **Волшебная палочка (`W`)** выделяет связанную область похожего цвета;
...
ZeTer Photo Editor пока не поддерживает полноценный PSD/PSB pipeline, smart objects, расширенное редактирование масок, direct-edit готовых Bézier-узлов/ручек, полный набор типов adjustment layers, CMYK/16/32-bit, RAW, сложное уточнение выделений, content-aware операции и полный набор Photoshop-compatible effects. Базовые layer masks и adjustment layers уже работают, а «Перо» создаёт сохраняемые cubic Bézier-контуры.

Следующие крупные направления развития: direct-edit Bézier-узлов и ручек, сложные выделения и маски, маски/режимы наложения для групп, расширенные типы adjustment layers, PSD import/export, более эффективная tiled-история растра и GPU/worker-ускорение для очень больших документов.