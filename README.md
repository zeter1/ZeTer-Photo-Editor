...
Самый простой вариант: дважды кликните `start.bat` или сразу `index.html`. Версия 1.40.0 специально собрана так, чтобы работать через обычный `file://` запуск — **Python, Node.js и локальный HTTP-сервер для использования редактора больше не нужны**.
...
PSD Import Stage 3 / Export Stage 4 и PSB Stage 7a дают layered round-trip, Stage 12e–12g сохраняют native RGB 16/32-bit typed channels и compatible high-depth composite, Stage 13a–13e закрывают native CMYK/color-management и real-world ICC corpus, Stage 14a–14c закрывают основную embedded Smart Object цепочку, Stage 15a–15d дают editable Photoshop Text/solid Shape mapping, а Stage 16a добавляет native basic Adjustment Layers. Brightness/Contrast, Exposure, Hue/Saturation и master Levels уже имеют editable bounded writeback; Curves сохраняют native points read-only до отдельного curve editor. Raster adjustment masks, Photoshop clipping/blend-if semantics, channel-specific Levels UI, editable Curves, Gradient/Pattern Fill renderer, multi-subpath boolean geometry, полная parity isolated groups/vector-mask composites и tiled/streaming pipeline ещё впереди.

Следующий imaging-этап: **Stage 16b — Adjustment Masks, Curves/Levels UI & Clipping Semantics**: добавить native raster-mask import/export для zero-bounds adjustment layers, editable Curves/channel-specific Levels и Photoshop clipping semantics без baked composite; после этого — isolated group/vector-mask parity и tiled/worker/GPU pipeline.