...
- **High-depth Raster Source Stage 12a**: импортированные RGB 16/32-bit PSD/PSB сохраняют исходный typed PixelBuffer внутри `.zpe` как bounded canonical binary source (до 48 МБ raw на документ) параллельно с RGBA8 preview. Этот foundation теперь используется Stage 12b для high-depth render/color, Stage 12d для базовых typed destructive operations и Stage 12e для native 16/32-bit PSD/PSB layer export; Canvas8 остаётся fallback-границей для неподдержанных операций.
- **High-depth Render Bridge Stage 12b**: raster layer с `highDepthSource` рендерится из сохранённого `Uint16Array`/`Float32Array`: exposure, gamma, temperature/tint, vibrance, highlights и shadows считаются до преобразования в 8-bit. Для 32-bit linear HDR используется ACES-style tone mapping + linear→sRGB display conversion, для 16-bit sRGB — high-precision linearized correction с clip display bridge. Декодированный typed source кэшируется bounded-кэшем на 2 слоя; после Stage 12d поддержанные base pixel tools продолжают работать в typed source, а неподдержанные retouch paths материализуются в Canvas8 fallback.
...
Самый простой вариант: дважды кликните `start.bat` или сразу `index.html`. Версия 1.26.0 специально собрана так, чтобы работать через обычный `file://` запуск — **Python, Node.js и локальный HTTP-сервер для использования редактора больше не нужны**.
