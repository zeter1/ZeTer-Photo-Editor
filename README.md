...
- **RGB 16-bit Import Stage 7c**: PSD/PSB RGB 16-bit Raw/RLE/ZIP декодируются в `Uint16Array` PixelBuffer с сохранением исходной точности на adapter boundary. Текущий ZPE/Canvas получает явный 8-bit preview, поэтому 16-bit editing/export пока не заявляются;
- **RGB 32-bit Float Import Stage 7d**: PSD/PSB RGB 32-bit/channel Raw/RLE/ZIP декодируются из big-endian IEEE-754 samples в `Float32Array` PixelBuffer, включая значения вне 0..1. Текущий Canvas preview временно ограничивает display-диапазон 0..1; HDR tone mapping/exposure и 32-bit editing/export пока не заявляются;
...
Самый простой вариант: дважды кликните `start.bat` или сразу `index.html`. Версия 1.22.0 специально собрана так, чтобы работать через обычный `file://` запуск — **Python, Node.js и локальный HTTP-сервер для использования редактора больше не нужны**.
...
PSD Import Stage 3 / Export Stage 4 и PSB Stage 7a дают ограниченный RGB/8-bit layered round-trip, а ZPE Smart Objects Stage 5a добавляет нативные embedded smart-object documents внутри `.zpe`, а Stage 11c — внутрипроектные linked instances с общим source ID и синхронным обновлением содержимого. Это ещё не полная Photoshop-семантика: PSB работает в текущих ZPE memory/Canvas limits; нет native PSD/PSB text/vector/smart-object round-trip, точной Photoshop parity для сложных adjustment/pass-through комбинаций внутри групп, native Photoshop adjustment-layer mapping, CMYK/16/32-bit editing pipeline и полного набора Photoshop-compatible effects. PixelBuffer Stage 7b умеет безопасно хранить RGB/CMYK 8/16/32-bit payload; Stage 7c декодирует RGB 16-bit PSD/PSB в `Uint16Array`, а Stage 7d — RGB 32-bit float в `Float32Array`. При открытии в редакторе high-depth данные пока переводятся в 8-bit preview (для 32-bit значения отображения ограничиваются 0..1), поэтому high-depth editing/export, HDR tone mapping/exposure, ICC/CMYK conversion и сохранение такой точности в `.zpe` ещё не включены. Экспорт сохраняет визуальный результат сложных ZPE-слоёв через raster preview; при наличии adjustment layers добавляется верхний `ZPE Composite Preview`, а исходные слои остаются скрытыми. Базовые raster layer masks и adjustment layers уже работают; Stage 10a–10d добавляет `.zpe` vector masks, прямое Bézier-редактирование и native PSD/PSB `vmsk` round-trip, а Stage 10e сохраняет Photoshop Path Information resources `2000..2997` в `document.paths`. Текстовые/shape/smart-object semantics по-прежнему в основном уходят в raster preview. Saved paths теперь доступны в отдельной панели «Контуры», но advanced Photoshop path metadata (например clipping-path designation и richer origination metadata) ещё не полностью отображается в UI.

Следующие крупные направления развития: PSD/PSB mapping для embedded smart objects/text/vector/adjustment layers, точная Photoshop parity для сложных group/adjustment interactions, внешние file-linked Smart Objects и Photoshop-native Smart Filter round-trip, clipping-path designation и richer Photoshop vector-origination metadata, сложные выделения и маски, 16/32-bit document/render pipeline, HDR tone mapping/exposure, CMYK/ICC color pipeline, настоящий tiled/streaming storage и GPU/worker-ускорение для очень больших документов.