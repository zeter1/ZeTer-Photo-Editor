...
- **HDR Preview Controls Stage 12c**: для каждого high-depth raster layer в Properties появились отдельные display-настройки **Tone map: Auto / Clip / ACES** и **Display exposure −6…+6 EV**. Они сохраняются в `.zpe`, не меняют исходный `Uint16Array`/`Float32Array` и входят в render-cache signature. `Auto` выбирает Clip для 16-bit и ACES для 32-bit float; display exposure применяется непосредственно перед tone mapping, поэтому можно исследовать HDR-света без изменения обычного layer Exposure. При переходе к destructive Canvas-редактированию выбранный HDR preview материализуется один раз в 8-bit base, а stale preview-настройки очищаются вместе с high-depth source.
- **Native High-depth Editing Foundation Stage 12d**: базовые destructive-операции над high-depth raster теперь могут работать прямо по `Uint16Array` / `Float32Array`, не материализуя слой в Canvas8: кисть, ластик, линия, заливка, очистка выделения и cut/clear по видимым слоям используют typed-buffer mutation path. RGB без alpha при необходимости ластика/очистки безопасно расширяется до straight RGBA той же bit depth; 16-bit samples остаются 16-bit, Float32 HDR headroom выше `1.0` сохраняется. Во время штриха renderer получает tone-mapped preview с `skipAdjustments`, а после commit новый high-depth source сериализуется обратно в `.zpe`; старый 8-bit fallback остаётся для retouch-инструментов (blur/clone/heal/smudge/dodge/burn) и случаев, когда RGBA-expansion не помещается в bounded high-depth budget.
...
Самый простой вариант: дважды кликните `start.bat` или сразу `index.html`. Версия 1.25.0 специально собрана так, чтобы работать через обычный `file://` запуск — **Python, Node.js и локальный HTTP-сервер для использования редактора больше не нужны**.
...
Следующие крупные направления развития: **Stage 12e high-depth PSD/PSB writer/export** для RGB 16/32-bit layer/composite channels, затем расширение native typed editing на retouch tools, CMYK/ICC color-management pipeline, PSD/PSB mapping для embedded smart objects/text/vector/adjustment layers, точная Photoshop parity для сложных group/adjustment interactions, внешние file-linked Smart Objects и Photoshop-native Smart Filter round-trip, clipping-path designation и richer Photoshop vector-origination metadata, tiled/streaming storage и GPU/worker-ускорение для очень больших документов.