...
- **CMYK / ICC Preview Foundation Stage 13a**: PSD/PSB importer теперь принимает RGB **и CMYK** 8/16/32-bit/channel, корректно отделяет четыре CMYK color planes от optional alpha и переводит Photoshop-inverted CMYK storage во внутренний ink-space PixelBuffer. Для CMYK-профилей реализован dependency-free ICC device→PCS→sRGB display bridge: читаются `A2B0/A2B1/A2B2` LUT-теги `mft1` / `mft2`, выполняется 4D multilinear CLUT interpolation, PCS Lab/XYZ переводится из D50 в display sRGB. Если профиль отсутствует, повреждён или использует пока неподдерживаемый `mAB`/MPE transform, UI явно использует unmanaged Device-CMYK approximation и предупреждает, что это не proof/press simulation. Native CMYK samples, включая 8-bit, сохраняются bounded-source внутри `.zpe`; renderer показывает созданный sRGB preview, а destructive RGB-editing честно растрирует его и снимает stale CMYK source. Native CMYK PSD/PSB writer/editing остаются следующим этапом Stage 13.
...
- **PSD Export Stage 4 + Stage 12e/12g**: экспортирует RGB `.psd` с отдельными слоями, Unicode-именами, opacity, visibility, поддерживаемыми blend modes, merged transparency и bitmap user masks. 8-bit использует PackBits/RLE; совместимые high-depth raster layers экспортируются native как 16-bit `Uint16` или 32-bit Float channels через `Lr16/Lr32`. Text/shape/transforms/filters/styles по-прежнему растрируются в preview соответствующего слоя, а adjustment layers сохраняют визуальный результат через отдельный верхний `ZPE Composite Preview`;
...
- **PSB Stage 7a + Stage 12e**: импорт/экспорт `.psb` (Photoshop Large Document Format, version 2) с 64-bit section/channel lengths; 8-bit сохраняет 32-bit RLE scanline counts, а 16/32-bit native channels используют Raw big-endian payload и `8B64` `Lr16/Lr32` layer-info blocks. Текущие ZPE safety limits по памяти/Canvas сохраняются, поэтому это совместимость формата, а не снятие лимитов до 300 000 px;
...
- **ICC Metadata Foundation Stage 7f + Stage 13a**: Image Resources разбирают ICC Profile resource `0x040F/1039` и ICC Untagged `0x0410/1041`, сохраняют raw profile bytes (до 4 МБ) и header summary. Stage 13a уже использует compatible CMYK `A2B*` `mft1/mft2` transforms для display-конверсии в sRGB; RGB profiles пока сохраняются как metadata без общего RGB→display ICC transform, а `mAB`/MPE остаются явным fallback;
...
Самый простой вариант: дважды кликните `start.bat` или сразу `index.html`. Версия 1.29.0 специально собрана так, чтобы работать через обычный `file://` запуск — **Python, Node.js и локальный HTTP-сервер для использования редактора больше не нужны**.
...
PSD Import Stage 3 / Export Stage 4 и PSB Stage 7a дают ограниченный layered round-trip, а Stage 12e добавляет native RGB 16/32-bit layer-channel export для совместимых high-depth raster layers. ZPE Smart Objects Stage 5a добавляет нативные embedded smart-object documents внутри `.zpe`, а Stage 11c — внутрипроектные linked instances с общим source ID и синхронным обновлением содержимого. Это ещё не полная Photoshop-семантика: PSB работает в текущих ZPE memory/Canvas limits; нет native PSD/PSB text/vector/smart-object round-trip, точной Photoshop parity для сложных adjustment/pass-through комбинаций внутри групп, native Photoshop adjustment-layer mapping и полного набора Photoshop-compatible effects. Stage 13a уже импортирует CMYK и строит profile-aware sRGB preview для ICC `mft1/mft2`, но native CMYK editing/export и полный ICC v4 `mAB`/MPE pipeline ещё не реализованы. Stage 12d сохраняет precision при базовых typed destructive operations, Stage 12e пишет native `Uint16`/`Float32` layer channels обратно в PSD/PSB, а Stage 12f переносит blur/clone/heal/smudge/dodge/burn в typed retouch path. Stage 12g убирает Canvas8 из совместимого multi-layer merged composite; transforms/effects/styles уже могут входить туда как честный 8-bit widened preview, но isolated groups, vector-mask composite, adjustment layers и другие несовместимые rasterization boundaries всё ещё используют явный Canvas8 fallback; RGB ICC-профиль пока сохраняется как metadata без общего display transform; CMYK Stage 13a делает реальный `A2B*` LUT preview-transform для поддерживаемых профилей, но native CMYK edit/export и soft-proofing ещё не включены. Экспорт сохраняет визуальный результат сложных ZPE-слоёв через raster preview; при наличии adjustment layers добавляется верхний `ZPE Composite Preview`, а исходные слои остаются скрытыми. Базовые raster layer masks и adjustment layers уже работают; Stage 10a–10d добавляет `.zpe` vector masks, прямое Bézier-редактирование и native PSD/PSB `vmsk` round-trip, а Stage 10e сохраняет Photoshop Path Information resources `2000..2997` в `document.paths`. Текстовые/shape/smart-object semantics по-прежнему в основном уходят в raster preview. Saved paths теперь доступны в отдельной панели «Контуры», но advanced Photoshop path metadata (например clipping-path designation и richer origination metadata) ещё не полностью отображается в UI.

Следующие крупные направления развития: **Stage 13b — расширенный ICC + native CMYK round-trip**: добавить ICC v4 `mAB`/MPE transforms, rendering-intent/display-profile policy, затем CMYK writer/export и native CMYK destructive/composite path без обязательной RGB rasterization. После этого — PSD/PSB mapping для embedded smart objects/text/vector/adjustment layers, typed vector-mask/isolated-group composite parity, внешние file-linked Smart Objects, Photoshop-native Smart Filter round-trip, richer path metadata, tiled/streaming storage и GPU/worker-ускорение.