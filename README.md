...
Самый простой вариант: дважды кликните `start.bat` или сразу `index.html`. Версия 1.36.0 специально собрана так, чтобы работать через обычный `file://` запуск — **Python, Node.js и локальный HTTP-сервер для использования редактора больше не нужны**.
...
PSD Import Stage 3 / Export Stage 4 и PSB Stage 7a дают layered round-trip, Stage 12e–12g сохраняют native RGB 16/32-bit typed channels и compatible high-depth composite, Stage 13a–13e закрывают native CMYK/color-management и real-world ICC corpus, Stage 14a–14c закрывают основную embedded Smart Object цепочку, а Stage 15a–15b уже импортируют horizontal Photoshop `TySh` как editable text layer, применяют first-run EngineData typography и позволяют менять содержимое single-run текста с native PSD/PSB writeback. Ограничение текста теперь локализовано на multi-run character/paragraph styling, изменение typography и vertical text — эти случаи остаются честным raster fallback. Shape/Vector Fill, Adjustment Layer mapping, полная parity isolated groups/vector-mask composites и tiled/streaming pipeline ещё впереди.

Следующий imaging-этап: **Stage 15c — Photoshop-native Shape / Vector Fill Mapping**: разобрать shape/vector-fill descriptors (`vmsk` + solid/gradient fill metadata), сопоставить поддержанные Photoshop shapes с editable ZPE shape layers и добавить безопасный native round-trip; затем перейти к Adjustment Layer mapping.