...
> **История изменений:** подробный журнал развития ZeTer Photo Editor — новые возможности, исправления ошибок, улучшения совместимости PSD/PSB и другие технические изменения — доступен в [CHANGELOG.md](CHANGELOG.md).
...
- **Полноценная работа со слоями:** растровые, текстовые, фигурные, корректирующие слои и Smart Objects; группы, вложенные группы, блокировка, видимость, непрозрачность и режимы наложения.
- **PSD/PSB-совместимость:** импорт и экспорт многослойных Photoshop-документов с сохранением поддерживаемой структуры, групп, масок, blend modes, ICC-метаданных и совместимых native-данных.
- **16-bit и 32-bit HDR pipeline:** импорт, хранение, редактирование и экспорт высокоточных RGB-данных без обязательного сведения всего документа к 8-bit.
- **CMYK и ICC color management:** поддержка CMYK PSD/PSB, встроенных ICC-профилей, преобразований CMYK↔RGB, display-profile policy и soft proofing.
- **Неразрушающее редактирование:** Adjustment Layers, маски слоёв, векторные маски, эффекты, цветокоррекция и редактируемые параметры без изменения исходных пикселей.
- **Photoshop Adjustment Layers:** поддерживаемые Brightness/Contrast, Exposure, Hue/Saturation, Levels, Curves, Invert, Posterize и Threshold импортируются как семантические корректирующие слои и для совместимых случаев сохраняются обратно в PSD/PSB.
- **Smart Objects:** сохранение и работа с поддерживаемыми embedded/linked Photoshop Smart Object metadata и preview без разрушения исходного содержимого.
- **Редактируемый текст и векторная графика:** текстовые слои, локальные шрифты, загрузка собственных шрифтов, прямоугольники, эллипсы, Bézier-контуры и инструмент «Перо».
...
- **Несколько документов во вкладках:** отдельное состояние, история, масштаб, выделение и активные контуры для каждого открытого документа.
- **Undo/Redo и защита данных:** ограниченная по памяти история, асинхронное сохранение растровых изменений и проверки против гонок при долгих операциях.
- **Аварийное восстановление:** несохранённые документы автоматически сохраняются в IndexedDB и могут быть восстановлены после перезапуска или сбоя.
- **Собственный формат проекта `.zpe`:** сохраняет структуру документа, слои, маски, эффекты, high-depth pixel data, цветовые профили и другие редактируемые данные.
- **Работа локально и онлайн:** редактор запускается прямо из `index.html` через `file://`, а обычная обработка изображений выполняется в браузере без обязательной загрузки файлов на внешний сервер.
- **Оптимизация больших документов:** лимиты памяти, bounded processing, асинхронное PNG-кодирование, `requestAnimationFrame`, coalesced pointer events и специализированные high-depth paths уменьшают зависания и лишнее потребление RAM.
...
- **Photoshop-native Text Layer Mapping Stage 15a**: PSD/PSB adapter bounded-разбирает `TySh` / TypeToolObjectSetting: text, affine transform, orientation, anti-alias policy, descriptor/warp identity и TypeTool bounds. Поддержанный horizontal Photoshop Type Layer импортируется как настоящий editable ZPE `text` layer, а исходный `TySh` хранится отдельно в `.zpe`. Пока text typography/size/scale/rotation не изменены, native PSD/PSB export возвращает `TySh`; перемещение слоя переписывает только affine `tx/ty`. При изменении текста или typography Stage 15a намеренно включает raster fallback, потому что `EngineData` style/text runs пока не переписываются — stale Photoshop text metadata не создаётся.
- **Photoshop EngineData Typography & Editable Text Writeback Stage 15b**: `TySh` теперь извлекает bounded `EngineData` tree и first/default style/paragraph runs: Photoshop font name, размер, faux bold/italic, fill color, paragraph alignment, leading, tracking, underline/strike-through и run lengths. Эти параметры реально применяются к ZPE text layer при импорте. Для single-style/single-paragraph-run текста изменение содержимого безопасно переписывает одновременно descriptor `Txt `, `/EngineDict /Editor /Text` и обе `RunLengthArray`, после чего PSD/PSB снова декодируется с тем же font/style/paragraph metadata. Multi-run text и изменение typography намеренно остаются raster fallback, чтобы не разрушать character/paragraph runs.
- **Photoshop-native Shape / Vector Fill Mapping Stage 15c**: adapter bounded-разбирает solid-fill shape metadata `SoCo`/`vscg` и Photoshop stroke descriptor `vstk`, а существующий `vmsk/vsms` path parser становится semantic geometry для настоящего editable ZPE `shape:'path'`. Поддержанный single-subpath solid shape получает native fill/stroke (`#00ffff` / `#ff00ff` в real fixture), Bezier/corner anchors и может менять path/move без потери Photoshop shape metadata. `vscg/vstk` bytes сохраняются opaque byte-for-byte, а vector mask пересобирается из текущих ZPE path points. Изменение fill/stroke, resize/scale/rotation, effects или filters честно переключает слой на raster fallback до descriptor rewrite следующего этапа.
- **Shape Descriptor Rewrite + Gradient/Pattern Fill Foundation Stage 15d**: solid Photoshop shapes теперь допускают native изменение fill/stroke/strokeWidth: adapter bounded-переписывает RGB doubles в `SoCo/vscg`, `strokeEnabled/fillEnabled`, `strokeStyleLineWidth` и nested RGB stroke content в `vstk`, не пересобирая descriptor целиком и не мутируя source blocks. Параллельно `GdFl`/`PtFl` добавлены в bounded parser: real Gradient Fill отдаёт angle/type/name/form/color+opacity stops, Pattern Fill — name/id/scale/link policy. Zero-bounds fill layers сохраняются в `fillLayers` metadata foundation, а canvas пока честно использует composite preview до отдельного editable gradient/pattern renderer.
- **Photoshop-native Adjustment Layer Mapping Stage 16a**: PSD/PSB adapter bounded-разбирает реальные `brit/CgEd`, `expA`, `hue2/hue `, `levl` и `curv` records и возвращает их отдельным zero-bounds semantic stack вместо потери как «пустых» слоёв. Brightness/Contrast, Exposure, Hue/Saturation и master Levels редактируются в Properties и bounded-патчат исходные native blocks; Curves импортируются и рендерятся по channel points, но Stage 16a сохраняет их native только пока points не изменены. Adjustment renderer работает поверх нижележащего composite, учитывает raster/vector masks и оставляет generic ZPE filters отдельным post-adjustment слоем. Если metadata/mask/edit нельзя безопасно представить в Photoshop record, export использует существующий честный composite fallback без stale metadata.
- **Adjustment Masks, Curves/Levels UI & Clipping Semantics Stage 16b**: zero-bounds Photoshop adjustment layers теперь сохраняют настоящий raster mask channel `-2` и clipping byte вместо composite fallback. Properties редактирует Master + Red/Green/Blue Levels и point-based Curves (`input:output`, 2–19 точек); writer обновляет channel-specific `levl` records и безопасно пересобирает point-based `curv` v1 + `Crv ` extra marker. Renderer ограничивает clipped adjustment alpha нижележащим base layer в том же sibling stack, а PSD/PSB writer возвращает document-sized adjustment masks и clipping metadata. Real MIT fixtures покрывают adjustment mask, clipping, multi-channel Levels и non-trivial Curves.
- **Native Invert / Posterize / Threshold Stage 16c**: PSD/PSB adapter распознаёт Photoshop `nvrt`, `post` и `thrs` как настоящие zero-bounds adjustment layers. Invert сохраняет parameterless native record, Posterize редактирует 2–255 уровней, Threshold — порог 1–255; semantic renderer использует Photoshop-compatible 8-bit quantization/luminance rules, маски и clipping продолжают работать через Stage 16b. Native writeback меняет только 16-bit payload `post/thrs` или сохраняет `nvrt` byte-identical, а .zpe sanitizer allow-list-ит новые kinds/blocks.
...
Самый простой вариант: дважды кликните `start.bat` или сразу `index.html`. Версия 1.42.0 специально собрана так, чтобы работать через обычный `file://` запуск — **Python, Node.js и локальный HTTP-сервер для использования редактора больше не нужны**.
...
При публикации по HTTP/HTTPS (в том числе через файловый менеджер uCoz) сборка использует `version.json` и отдельный build ID. Манифест проверяется с `cache: no-store` и уникальным query-параметром; если на сервере появился новый build, страница один раз перезагружается с `zpe_build=<build-id>`, а CSS/JS получают тот же build ID в URL. Поэтому после внедрения этого механизма обычным пользователям не нужен `Ctrl+F5` для следующих обновлений. Режим `file://` эту сетевую проверку не выполняет и продолжает запускаться напрямую.

Для безопасной ручной выкладки сначала загружайте `index.html`, `src/app.bundle.js`, `src/styles.css` и остальные файлы приложения, а `version.json` — **последним**: он служит сигналом, что новый build уже полностью лежит на сервере. При самом первом развёртывании этого механизма браузер, в котором ещё закэширован старый `index.html` без update-check, может потребовать один последний жёсткий refresh; начиная со следующей выкладки обновление определяется автоматически.
