...
- **RGB 16-bit Import Stage 7c**: PSD/PSB RGB 16-bit Raw/RLE и ZIP без prediction декодируются в `Uint16Array` PixelBuffer с сохранением исходной точности на adapter boundary. Текущий ZPE/Canvas получает явный 8-bit preview, поэтому 16-bit editing/export пока не заявляются; ZIP prediction для 16-bit отклоняется явно;
- **RGB 32-bit Float Import Stage 7d**: PSD/PSB RGB 32-bit/channel Raw/RLE и ZIP без prediction декодируются из big-endian IEEE-754 samples в `Float32Array` PixelBuffer, включая значения вне 0..1. Текущий Canvas preview временно ограничивает display-диапазон 0..1; HDR tone mapping/exposure, 32-bit editing/export и ZIP prediction для high-depth пока не заявляются;
