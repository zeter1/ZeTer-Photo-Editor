...
Самый простой вариант: дважды кликните `start.bat` или сразу `index.html`. Версия 1.34.0 специально собрана так, чтобы работать через обычный `file://` запуск — **Python, Node.js и локальный HTTP-сервер для использования редактора больше не нужны**.
...
PSD Import Stage 3 / Export Stage 4 и PSB Stage 7a дают layered round-trip, Stage 12e–12g сохраняют native RGB 16/32-bit typed channels и compatible high-depth composite, Stage 13a–13e закрывают native CMYK/color-management и real-world ICC corpus, а Stage 14a–14b сохраняют Photoshop Smart Object metadata и извлекают supported embedded assets в editable content-tab. Ограничение Smart Objects теперь локализовано: после редактирования embedded content ZPE пока не переписывает `lnk*` payload/descriptor, поэтому экспорт безопасно растрирует изменённый экземпляр. Также ещё нет native Text/Shape/Adjustment mapping, полной parity isolated groups/vector-mask composites и tiled/streaming pipeline.

Следующий imaging-этап: **Stage 14c — Smart Object Embedded Asset Rewrite & Resource Rebuild**: после редактирования content-tab сериализовать supported embedded bitmap/PSD обратно в `liFD` record, обновлять `datasize`/linked resources и согласованные `SoLd/PlLd` identity/preview без raster fallback. Затем — Photoshop-native Text/Shape/Adjustment mapping.