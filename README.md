...
- **Перо (`P`)** строит сохраняемый векторный контур, **Магнитное лассо (`A`)** прослеживает границу промежуточными точками между кликами, а **Волшебная палочка (`W`)** выделяет связанную область похожего цвета;
...
- загрузка `.zpe` жёстче нормализует повреждённые/аномальные параметры слоя, duplicate ID и неподдерживаемые blend mode.
...
ZeTer Photo Editor пока не поддерживает полноценный PSD/PSB pipeline, smart objects, маски слоёв, продвинутое редактирование Bézier-контуров, adjustment layers, CMYK/16/32-bit, RAW, сложное уточнение выделений, content-aware операции и полный набор Photoshop-compatible effects. Базовый инструмент «Перо» уже создаёт сохраняемые векторные контуры.

Следующие крупные направления развития: сложные выделения и маски, маски/режимы наложения для групп, non-destructive adjustment layers, PSD import/export, более эффективная tiled-история растра и GPU/worker-ускорение для очень больших документов.