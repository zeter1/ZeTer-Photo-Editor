...
const toolbarController=await readFile(new URL('../src/ui/toolbar-controller.js',import.meta.url),'utf8');
...
test('every toolbar tool receives a rich accessible tooltip instead of a short native title',()=>{
  ...
  assert.match(toolbarController,/button\.removeAttribute\('title'\)/);
  assert.match(toolbarController,/button\.setAttribute\('aria-describedby', tooltip\.id\)/);
