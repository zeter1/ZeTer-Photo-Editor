...
test('clone stamp requires Alt-click source and paints from an immutable stroke snapshot', () => {
  ...
  assert.match(main, /cloneSource=\{layerId:layer\.id/);
  assert.match(main, /function prepareCloneStroke\(layer, destinationPoint\)/);
  assert.match(main, /cloneSnapshotCanvas\.getContext\('2d'/);
  assert.match(main, /function cloneStrokeSegment\(from,to,offset,pointerEvent=null,healing=false\)/);
...
test('retouch tools edit only an existing raster layer and keep dedicated history labels', () => {
  ...
  assert.match(main, /function applyToneDab\(layer,point,pointerEvent=null,brighten=true\)/);
  assert.match(main, /applyToneBrushPixels\(imageData\.data/);
...
test('retouch dabs use feathered masks and magnetic lasso traces intermediate edge points', () => {
  assert.match(main, /function applyFeatherMask\(ctx,centerX,centerY,radius\)/);
  assert.match(main, /applyFeatherMask\(scratchCtx,point\.x-left,point\.y-top,radius\)/);
